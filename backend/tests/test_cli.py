"""autoclip CLI / local_runner：translated、projectdirectorytranslated、translated、progresstranslated（translated、translated）"""
import json
import os
import subprocess
import sys
...
def data_dir(tmp_path, monkeypatch):
    """translateddirectory + translated SQLite，translatedusertranslateddirectory"""
    d = tmp_path / "data"
...
def _fake_project(data_dir: Path, project_id: str = "p1", name: str = "translated") -> Path:
    ...
    clips = [
        {"id": "1", "generated_title": "translated", "start_time": "00:00:01,000", "end_time": "00:00:05,000", "final_score": 0.62},
        {"id": "2", "generated_title": "translated", "start_time": "00:01:00,000", "end_time": "00:01:30,000", "final_score": 9.1},
    ]
    ...
    (pdir / "metadata" / "collections_metadata.json").write_text(
        json.dumps([{"id": "1", "collection_title": "collectiontranslated", "clip_ids": ["1", "2"]}], ensure_ascii=False), encoding="utf-8")
    clip_paths = [str(pdir / "output" / "clips" / "1_translated.mp4"), str(pdir / "output" / "clips" / "2_translated.mp4")]
    (pdir / "output" / "step6_video_output.json").write_text(
        json.dumps({"clip_paths": clip_paths, "collection_paths": [str(pdir / "output" / "collections" / "collectiontranslated.mp4")]}, ensure_ascii=False),
        encoding="utf-8")
    return pdir
...
def test_summarize_project_normalizes_scores_and_paths(data_dir):
    ...
    assert s["name"] == "translated"
    assert s["counts"] == {"clips": 2, "collections": 1}
    # 0–1 and 0–10 translatedonetranslated 0–100，translatedbytranslated
    assert [c["score_100"] for c in s["clips"]] == [91, 62]
    assert s["clips"][0]["file"].endswith("2_translated.mp4")
    assert s["collections"][0]["file"].endswith("collectiontranslated.mp4")


def test_list_projects_reads_filesystem(data_dir):
    from backend.services.local_runner import list_projects

    _fake_project(data_dir, "p1", "translated")
    _fake_project(data_dir, "p2", "translated")
    items = list_projects()
    assert {i["name"] for i in items} == {"translated", "translated"}
    assert all(i["status"] == "completed" and i["clips"] == 2 for i in items)


@pytest.fixture
def isolated_db(data_dir, monkeypatch):
    """backend.core.database 's engine Is import translated's，cantranslated'stesttranslated；thistranslated。"""
    from sqlalchemy import create_engine
    from sqlalchemy.orm import sessionmaker
    from sqlalchemy.pool import StaticPool
    import backend.core.database as database
    from backend.models import project as _  # noqa: F401  ensuremodeltranslated Base
...
def test_prepare_project_links_video_and_registers_db(data_dir, tmp_path, isolated_db, monkeypatch):
    ...
    assert meta["project_name"] == "translatedtest" and meta["source"]["via"] == "cli"

    from backend.models.project import Project
    db = isolated_db()
    try:
        row = db.query(Project).filter(Project.id == req.project_id).first()
        assert row is not None and row.name == "translatedtest"
...
def test_progress_listener_receives_payload(data_dir):
    ...
    try:
        sp.emit_progress("proj-x", "ANALYZE", "translated", subpercent=50)
    finally:
...
def test_mcp_get_project_falls_back_to_disk(data_dir):
    ...
    _fake_project(data_dir, "p9", "translatedproject")
    r = mcp_server.get_job_status("p9")
    assert r["ok"] is True and r["status"] == "completed" and r["result"]["name"] == "translatedproject"
