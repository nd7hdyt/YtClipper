...
const FileUpload: React.FC<FileUploadProps> = ({ onUploadSuccess }) => {
  ...
  const onDrop = (acceptedFiles: File[]) => {
    ...
    acceptedFiles.forEach(file => {
      ...
      if (['mp4', 'avi', 'mov', 'mkv', 'webm'].includes(extension || '')) {
        newFiles.video = file
        // Auto-fill the project name from the video filename (without extension)
        // Update the name every time a new video file is picked
  ...
  const handleUpload = async () => {
    ...
    try {
      // Simulated upload progress for a more realistic display
      const progressInterval = setInterval(() => {
        setUploadProgress(prev => {
          ...
          // Decelerating increments to mimic real upload progress
      ...
      console.log('Uploading files:', {
        ...
        srt_file: files.srt?.name || '(will auto-generate with speech recognition)',
      ...
      console.log('Upload succeeded, project:', newProject)
      ...
      // Friendlier errors by type
      if (error.response?.status === 413) {
        errorMessage = 'File too large, please choose a smaller video'
        errorType = 'warning'
        ...
        if (error.response?.data?.detail) {
          ...
          errorMessage = 'File issue, please check and retry'
        }
      } else if (error.response?.status === 500) {
        errorMessage = 'Server error while processing, please retry later'
      ...
      // Retry hint on network errors
      if (error.code === 'ECONNABORTED' || error.response?.status >= 500) {
        message.info('If this persists, check your connection or contact support', 5)
      }
    } finally {
      setUploading(false)
    }
  ...
  return (
    <div style={{
      ...
      >
        ...
        <div>
          ...
          <Text style={{ color: 'var(--ac-sub)', fontSize: '14px', lineHeight: '1.5' }}>
            Supports MP4, AVI, MOV, MKV, WebM. <Text style={{ color: '#52c41a', fontWeight: 600 }}>Optionally import subtitles (.srt) or auto-generate with AI</Text>
          </Text>
        </div>
      </div>

      {/* Project name - shown only after a file is picked */}
      {files.video && (
        <div style={{ marginBottom: '16px' }}>
          ...
          <Input
            ...
            placeholder="Enter a project name for your video"
            style={{ 
              height: '40px',
              borderRadius: '12px',
              fontSize: '14px',
      ...
      {/* Category select - shown only after a file is picked */}
      {files.video && (
        <div style={{ marginBottom: '16px' }}>
          <Text strong style={{ color: '#ffffff', fontSize: '14px', marginBottom: '8px', display: 'block' }}>
            Category
          ...
          {files.video && !files.srt && (
            <div style={{
              ...
              <Text style={{ color: '#52c41a', fontSize: '14px', fontWeight: 500 }}>
                Subtitles will be auto-generated with AI speech recognition
              </Text>
            </div>
          )}
        </div>
      ...
      {/* Upload button - shown only after a file is picked */}
      {files.video && (
