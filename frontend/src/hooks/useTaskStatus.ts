...
export interface TaskStatus {
  ...
  error?: string;
  updatedAt: string;
  project_id?: string; // addprojectIDEN
}
...
export const useTaskStatus = () => {
  console.log('🔧 useTaskStatus HookEN');
  ...
  const loadProjectTasks = useCallback(async (projectId: string) => {
    console.log('📤 ENprojecttask:', projectId);
    setLoading(true);
    try {
      const response = await fetch(`/api/v1/tasks/project/${projectId}`);
      console.log('📡 APIENStatus:', response.status);
      
      if (response.ok) {
        const data = await response.json();
        const projectTasks = data.data.tasks || [];
        console.log('📋 fetchtotaskEN:', projectTasks.length);
        
        setTasks(prev => {
          const newTasks = new Map(prev);
          projectTasks.forEach((task: any) => {
            console.log('📝 Add task:', task.task_id, task.status, task.progress);
        ...
        console.error('❌ APIENFailed:', response.status, response.statusText);
      }
    } catch (error) {
      console.error('❌ ENprojecttaskFailed:', error);
    } finally {
      setLoading(false);
      console.log('✅ taskENCompleted');
    }
  }, []); // EN，EN
